...
echo "3. install AddHost"
echo "4. install insMariaDB10_1.sh (centos 7)"
echo "5. install PHP7.3 (centos 7 or centos 8)"
...
3) bash <(curl -L "$sources"shell/addHost.sh) ;;	#需要域名
4) bash <(curl -L "$sources"shell/insMariaDB10_1.sh) ;;
5) bash <(curl -L "$sources"shell/insPHP73.sh) ;;
